...
- **`GET …/subtasks` response shape is `{"subtaskGroups": [{ id, task_id, subtasks: [...] }]}`** — **not** a top-level `{"subtasks": [...]}`. Pulling `.subtasks` off the root returns empty even when subtasks exist, which looks like "the close didn't land" but is just a client-side parse mistake. Always iterate `subtaskGroups[*].subtasks[*]`. Each subtask carries `meta.subtask_group_id` so you can route it back to its group.
